...
//			if(st.isStatus())
//				m++;
//		Log.printLine("Size smartThings list: "+AppExemplo2.getSmartThings().size());
...
//						CloudSim.getEntity(next.getSource()).getFutureQueue().remove(next);
	...
	public static void runStop() {
		...
		Log.printLine("Finished: "+Calendar.getInstance().getTime());
		Log.printLine("Total Time: "+countador);
	}
	...
	public static boolean cancelAll(int src, Predicate p) {
		...
		while (iter.hasNext()) {
			ev = iter.next();
			if (ev.getSource() == src && p.match(ev)) {
//				Log.printLine("src: "+src+" ev: "+ev.toString()+
...
//						" sched by: " + ev.scheduledBy() + " data :" + ev.getData());
	...
	public static boolean cancelAll(FutureQueue sourceQueue) {
		...
		while (iter.hasNext()) {
			ev = iter.next();
//				Log.printLine("src: "+ev.scheduledBy() +" ev: "+ev.toString()+
//					" type: " + ev.getType() + " ev time: "+ ev.eventTime() +
//					" sched by: " + ev.scheduledBy() + " data :" + ev.getData());
//			Log.printLine(future.remove(ev));
			future.remove(ev);
			iter.remove();
		}
		return previousSize < future.size();

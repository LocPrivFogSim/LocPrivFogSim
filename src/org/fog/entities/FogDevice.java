...
public class FogDevice extends PowerDatacenter {
	...
	private void notifyObservers(MobileDevice mobileDevice, int eventType, String event) {
		if (!CloudSim.running())
			return;
			
		for (Attacker observer : observerList) {
			observer.update(this, mobileDevice, eventType, event);
		}
	}
	...
	public void setSmartThings(MobileDevice st, int eventType, int action) {//myiFogSim
		if(action == Policies.ADD){
			this.smartThings.add(st);
			notifyObservers(st, eventType, "add");
		}
		else if (action == Policies.REMOVE){
			this.smartThings.remove(st);
			notifyObservers(st, eventType, "remove");
		}
	}
	...
	public void addOffloadingTask(MobileDevice mobileDevice, OffloadingTask task) {
		...
		if (count == 1)
			notifyObservers(mobileDevice, EventType.OFFLOADING, "add");
	}
	...
	public void removeOffloadingTask(MobileDevice mobileDevice, OffloadingTask task) {
		...
		if (count == 0)
			notifyObservers(mobileDevice, EventType.OFFLOADING, "remove");
	}
...
//						sendNow(mobileController.getId(), FogEvents.APP_SUBMIT, app);
	...
	public boolean connectServerCloudletSmartThing(MobileDevice st){
		...
		setSmartThings(st, EventType.MIGRATION, Policies.ADD);
	...
	public boolean desconnectServerCloudletSmartThing(MobileDevice st){
		...
		//		}
		setSmartThings(st, EventType.MIGRATION, Policies.REMOVE); //it'll remove the smartThing from serverCloudlets-smartThing's set
...
//			}
	...
	private static void saveMigration(MobileDevice st){
		Log.printLine("MIGRATION " +st.getMyId() + " Position: " + st.getCoord().getCoordX() + ", " + st.getCoord().getCoordY() + " Direction: " + st.getDirection() + " Speed: " + st.getSpeed());

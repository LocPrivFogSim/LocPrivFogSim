...
public class ModulePlacementEdgewards extends ModulePlacement{
	...
	public ModulePlacementEdgewards(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, 
		...
		Log.printLine(getCurrentModuleInstanceNum());
	...
	protected void mapModules() {
		...
		for(int deviceId : getCurrentModuleMap().keySet()){
			Log.printLine(getFogDeviceById(deviceId).getName() + "--->"+getCurrentModuleMap().get(deviceId));
	...
	private List<String> getModulesToPlace(List<String> placedOperators){
		Log.printLine("Placed Modules : "+placedOperators);
		...
		Log.printLine("Modules to place : "+modulesToPlace);
	...
	private void placeModulesInPath(List<Integer> path) {
		...
		for(Integer deviceId : path){
			Log.printLine("On device "+CloudSim.getEntityName(deviceId));
			...
			Log.printLine("Currently placed operators = "+placedOperators);
					...
					for(Pair<String, String> pair : map.keySet()){
						if(pair.getFirst().equals(edge.getTupleType())){
							...
							if(!appEdgeToRate.containsKey(outputEdge) || appEdgeToRate.get(outputEdge)!=outputRate){
								Log.printLine(outputEdge.getSource()+"----->"+outputEdge.getDestination()+" : "+outputRate);
								changed = true;
							}
			...
			while(operatorsToPlace.size() > 0){
				Log.printLine("BEGINNING OF LOOP : Operators to Place : "+operatorsToPlace);
				...
				Log.printLine(operatorName+" XXXXX Upstream Device : "+CloudSim.getEntityName(upsteamDeviceId));
				if(upsteamDeviceId > 0){
					Log.printLine(CloudSim.getEntityName(deviceId)+" : "+getCurrentModuleInstanceNum().get(upsteamDeviceId).get(operatorName)+" modules of Operator "+operatorName+" is already placed on device "+CloudSim.getEntityName(upsteamDeviceId));
					if(upsteamDeviceId==deviceId){
						...
						for(AppEdge edge : getApplication().getEdges()){		// take all incoming edges
							if(edge.getDestination().equals(operatorName)){
								...
								/*Log.printLine("Tuple type = "+edge.getTupleType());
								Log.printLine("Rate = "+rate);
								Log.printLine("Cpu Load = "+rate*edge.getTupleCpuLength());*/
							}
						}
						if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){
							/*Log.printLine("Placement of operator "+operatorName+ "NOT POSSIBLE on device "+device.getName());
							Log.printLine("CPU load = "+totalCpuLoad);
							Log.printLine("Current CPU load = "+getCurrentCpuLoad().get(deviceId));
							Log.printLine("Max mips = "+device.getHost().getTotalMips());*/
							Log.printLine("Need to shift module "+operatorName+" somewhere upstream.");
							Log.printLine("Operators to place = "+operatorsToPlace);
							Log.printLine(getCurrentModuleInstanceNum());
							...
							Log.printLine("---------------------------------------------------");
							Log.printLine("Operators to place after shift = "+operatorsToPlace);
							Log.printLine("Instance map after shifting = "+getCurrentModuleInstanceNum());
						} else{
							placedOperators.add(operatorName);
							...
							Log.printLine("You can rest in peace. "+operatorName+" can be created in "+device.getName());
						}
					}
				}else{
					Log.printLine("+++++++++++++++++++++++++");
					...
					for(AppEdge edge : getApplication().getEdges()){		// take all incoming edges
						if(edge.getDestination().equals(operatorName)){
							Log.printLine(edge.getTupleType());
							double rate = appEdgeToRate.get(edge);
							totalCpuLoad += rate*edge.getTupleCpuLength();
							Log.printLine("Tuple type = "+edge.getTupleType());
							Log.printLine("Rate = "+rate);
					...
					Log.printLine("Trying to place module "+operatorName);
					...
					if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){
						...
						Log.printLine("Placement of operator "+operatorName+ " on device "+device.getName());
						getCurrentCpuLoad().put(deviceId, totalCpuLoad + getCurrentCpuLoad().get(deviceId));

						Log.printLine("Updated CPU load = "+getCurrentCpuLoad().get(deviceId));
						...
						Log.printLine("Number of instances for operator "+operatorName+" = "+max);
	...
	private List<String> shiftModuleNorth(String moduleName, double cpuLoad, Integer deviceId, List<String> operatorsToPlace) {
		...
		Log.printLine("------------------------------------------------");
		Log.printLine("Shifting module "+moduleName +" northwards.");
		
		List<String> modulesToShift = findModulesToShift(moduleName, deviceId);
		
		Log.printLine("Modules to shift northwards : "+modulesToShift);
		...
		Log.printLine("Module instances to shift northwards : "+moduleToNumInstances);
		
		int id = getParentDevice(deviceId);
		while(true){
			if(id==-1){
				Log.printLine("Could not place modules "+modulesToShift+" northwards.");
				break;
			}
			Log.printLine("Now on device "+CloudSim.getEntityName(id));
			...
			if(getCurrentCpuLoad().get(id) + totalCpuLoad > fogDevice.getHost().getTotalMips()){
				...
				Log.printLine("CPU load after operator removal on device "+CloudSim.getEntityName(id)+" = "+getCurrentCpuLoad().get(id));
				modulesToShift = _modulesToShift;
				id = getParentDevice(id);
			} else{
				// can place the modules here (@ id)
				Log.printLine("Can place modules "+modulesToShift+ "on device "+CloudSim.getEntityName(id));
				double totalLoad = 0;
				for(String module : loadMap.keySet()){
					...
					Log.printLine("Final module to num instances : "+moduleToNumInstances);
					...
					String module_ = module;
						...
						Log.printLine("Placing "+finalNumInstances+" on "+CloudSim.getEntityName(id));
						getCurrentModuleInstanceNum().get(id).put(module_, finalNumInstances);
					//}
				}
				getCurrentCpuLoad().put(id, totalLoad);
				Log.printLine("FINALLY placed "+loadMap.keySet()+" at device "+CloudSim.getEntityName(id));
				operatorsToPlace.removeAll(loadMap.keySet());
				Log.printLine("CPU load on device "+CloudSim.getEntityName(id)+" = "+getCurrentCpuLoad().get(id));
				List<String> placedOperators = new ArrayList<String>();
	...
	private int isPlacedUpstream(String operatorName, List<Integer> path) {
		Log.printLine(path);
		for(Integer i : path)Log.printLine(CloudSim.getEntityName(i));

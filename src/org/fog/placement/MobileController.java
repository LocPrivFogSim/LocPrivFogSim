	...
	private void checkNewStep() {
		...
		for(MobileDevice st: getSmartThings()){
			...
			if(st.getSourceAp()!=null)
			{
				...
				Log.printLine(st.getName()+"\t"+ st.getCoord().getCoordX()+"\t"+st.getCoord().getCoordY());
				Log.printLine(st.getSourceAp().getName()+ "\t"+st.getSourceAp().getCoord().getCoordX()+"\t"+st.getSourceAp().getCoord().getCoordY());
				Log.printLine(Distances.checkDistance(st.getCoord(), st.getSourceAp().getCoord()));
...
//									sendNow(st.getDestinationAp().getServerCloudlet().getId(),MobileEvents.MAKE_DECISION_MIGRATION,st);
			...
			else
			{
				...
				if(ApDevice.connectApSmartThing(getApDevices(), st, getRand().nextDouble())){
					...
					Log.printLine(st.getName()+"\t"+ st.getCoord().getCoordX()+"\t"+st.getCoord().getCoordY());
					Log.printLine(st.getSourceAp().getName()+ "\t"+st.getSourceAp().getCoord().getCoordX()+"\t"+st.getSourceAp().getCoord().getCoordY());
					Log.printLine("Distance: " + Distances.checkDistance(st.getCoord(), st.getSourceAp().getCoord()));
	...
	private static void saveHandOff(MobileDevice st){
		Log.printLine("HANDOFF "+st.getMyId() + " Position: " + st.getCoord().getCoordX() + ", " + st.getCoord().getCoordY() + " Direction: " + st.getDirection() + " Speed: " + st.getSpeed());
		try(FileWriter fw = new FileWriter(st.getMyId()+"handoff.txt", true);
			    BufferedWriter bw = new BufferedWriter(fw);
			    PrintWriter out = new PrintWriter(bw))
		{
			out.println(st.getMyId() + "\t" + CloudSim.clock() + "\t" + st.getCoord().getCoordX() + "\t" + st.getCoord().getCoordY() + "\t" + st.getDirection() + "\t" + st.getSpeed() + "\t" + st.getSourceAp().getName()+ "\t" + st.getDestinationAp().getName());
		}

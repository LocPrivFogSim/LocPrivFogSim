...
public class LiveMigration implements VmMigrationTechnique {
	...
	public void verifyPoints(MobileDevice smartThing, int relativePosition) {
		// TODO Auto-generated method stub
			...
			smartThing.setMigZone(migrationZoneFunction(smartThing.getPosition().getDirection(), relativePosition));
	}
	...
	public boolean migPointPolicyFunction(int policy, MobileDevice smartThing) {
		// TODO Auto-generated method stub
		double distance = Distances.checkDistance(smartThing.getSourceAp().getPosition().getCoordinate(), smartThing.getPosition().getCoordinate());
		...
		if(policy == Policies.FIXED_MIGRATION_POINT){	
			...
			return migrationPointFunction(distance, smartThing.getMigTime(), smartThing.getPosition().getSpeed());//relative according smartThing's speed
		}			
	}

...
//				st.setStatus(false);
				if(st.getSourceAp()==null){
					...
					if(st.getSourceServerCloudlet() != null){
						st.getSourceServerCloudlet().setSmartThings(st, EventType.MIGRATION, Policies.REMOVE); //it'll remove the smartThing from serverCloudlets-smartThing's set
					}
					st.getSourceAp().setSmartThings(st, EventType.MIGRATION, Policies.REMOVE);//it'll remove the smartThing from ap-smartThing's set
					LogMobile.debug("NextStep.java", st.getName()+" was removed!");
					smartThings.remove(st);
				}
			}
			else{
				Log.printLine("MobileDevice - Move: " + st.getName() + "\t" + st.getCoord().getCoordX() + "\t" + st.getCoord().getCoordY() + "\t" + CloudSim.clock() + "\t" + Calendar.getInstance().getTime());
				saveMobility(st);
			}
		}

			...
			for (FogDevice sc1 : serverCloudlets) {
				...
				if (sc.getUplinkBandwidth() < sc1.getDownlinkBandwidth()) {
					...
					NetworkTopology.addLink(sc.getId(), sc1.getId(),
							sc.getUplinkBandwidth(), (linha+coluna)*getLatencyBetweenCloudlets()+getRand().nextDouble());
				} else {
					net.put(sc1, sc1.getDownlinkBandwidth());
					NetworkTopology.addLink(sc.getId(), sc1.getId(),
							sc1.getDownlinkBandwidth(), (linha+coluna)*getLatencyBetweenCloudlets()+getRand().nextDouble());
				}
				j++;
			}
			i++;
